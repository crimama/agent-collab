...
each assigned to 'claude' or 'codex'.
"""
...
{{
  ...
  "tasks": [
    {{
      ...
      "prompt": "Detailed, self-contained prompt for the agent. Include filenames and context.",
      "agent": "claude",
      "depends_on": [],
      "parallel": false
    }}
...
def generate_plan(goal: str, cwd: str = ".") -> dict:
    """Call Claude to decompose `goal` into a structured plan (JSON dict)."""
    ...
    # Extract JSON block (Claude may wrap it in markdown fences)
    match = re.search(r"\{[\s\S]*\}", raw)
    if not match:
        raise ValueError(
            f"Planner could not parse JSON from Claude output.\n\n{raw[:500]}"
        )
