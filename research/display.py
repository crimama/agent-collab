...
import time
import threading
from state import RoundResult, StepResult
...
def print_session_header(goal: str, total_rounds: int) -> None:
    ...
    print(_c("║", "magenta") +
          _c("  AI RESEARCH MODE", "bold") +
          _c(f"  —  {total_rounds} round(s)", "dim") +
          " " * max(0, w - 23 - len(str(total_rounds)) - len(goal[:40])) +
          _c("║", "magenta"))
...
def print_step_result(step: StepResult) -> None:
    ...
    t = f"{step.duration_s:.1f}s"
    agents_used = {o.agent for o in step.outputs}
    print(_c(f"\n  ✓ {step.step_name} complete  ({t})", color) +
